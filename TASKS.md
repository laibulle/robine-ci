...
- [~] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, and graph reconciliation are implemented. A Rust runtime worker claims SQL outbox and durable dispatch work concurrently, applies idempotent internal projections, atomically hands each local attempt to a unique durable execution job, retries with capped stable jitter, dead-letters after ten failures, and repairs missing execution handoffs. Workflow parsing, local execution-job consumption, offer push transport and attempt transfers, dependency reruns, and external projection delivery remain.)
