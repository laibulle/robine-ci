...
- [~] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, and graph reconciliation are implemented. Rust runtime workers claim SQL outbox, durable dispatch, and local execution work; atomically hand each local attempt to a unique execution job; reconstruct authoritative execution input; persist ordered lifecycle and terminal graph state; recover terminal-before-completion crashes; retry with capped stable jitter; dead-letter after ten failures; and repair missing handoffs. A framework-independent Rust execution crate owns image acquisition and a hardened, labeled, bounded Docker lifecycle with sequential workspace semantics, timeouts, cleanup, and orphan selection. Workflow parsing, streaming/in-flight cancellation/source/built-in/service/secret parity, offer push transport and attempt transfers, dependency reruns, and external projection delivery remain.)
