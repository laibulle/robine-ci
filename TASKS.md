...
- [x] Select SQLx for PostgreSQL access, prove reads against the historical schema, and bootstrap or validate the final schema from the Rust-owned SQL baseline.
- [~] Add CI gates for Rust formatting, Clippy, tests, dependency auditing, and license policy. (Formatting, strict Clippy, workspace tests, PostgreSQL compatibility tests, coverage, and `cargo audit` are wired; automated license-policy enforcement remains.)

### RUST-002 — Replace the control plane by verified vertical slices

- **Spec:** [PLAT-004](docs/specs/platform/plat-004-rust-actix-migration.md)
- **Depends on:** RUST-001
- [~] Port database access, transactions, tenant isolation, identity, and sessions. (SQLx tenant transactions, first-admin bootstrap, compatible local sign-in, cookie and bearer session lifecycle, OIDC discovery/PKCE/callback validation with collision-safe provisioning, administrator-only role administration with last-admin protection, and authenticated pipeline/workflow/job/log/artifact/repository projections are implemented; OIDC key-rotation recovery and remaining operational projections remain.)
- [~] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, and graph reconciliation are implemented. Rust runtime workers claim SQL outbox, durable dispatch, and local execution work; atomically hand each local attempt to a unique execution job; reconstruct authoritative execution input; persist ordered lifecycle, bounded stdout/stderr chunks, and terminal graph state; poll durable cancellation during active commands; recover execution/log cursors and terminal-before-completion crashes; retry with capped stable jitter; dead-letter after ten failures; and repair missing handoffs. A framework-independent Rust execution crate owns image acquisition and a hardened, labeled, bounded Docker lifecycle with sequential workspace semantics, private service networks, digest-pinned TCP readiness, active service liveness, redacted 64 KiB diagnostic tails, timeouts, cancellation, cleanup, and orphan selection. Exact-SHA checkout uses tenant-trusted authenticated provider archives. Cache restore/save and artifact upload/download run at their declared step positions through tenant-scoped, digest-verified local or S3-compatible blobs and quota-locked PostgreSQL metadata. An hourly bounded worker prunes expired metadata/logs, delays garbage through durable candidates, rechecks references, inventories physical objects, stages orphans only from complete inventories, and removes abandoned temporary files. A pure Rust schema-v1 workflow crate parses the shared valid/invalid YAML corpus with source-located diagnostics, enforces typed trigger inputs, validates artifact dependencies, expands matrices, and supplies canonical immutable-revision jobs to pipeline creation. Pure bounded reusable-workflow composition namespaces nested graphs, isolates normalized call inputs, rejects missing/cyclic/excessive source graphs, and retains only reachable exact-revision sources. Actix returns structured workflow diagnostics and the runtime reads all documented workflow-limit environment variables. Authenticated heartbeat polling and WebSocket delivery discover pending offers; acceptance deadlines, explicit accept/reject, source and selected-secret delivery, ordered lifecycle/log callbacks, and bounded cache/artifact transfers are durable and attempt-scoped. Dependency reruns and external projection delivery remain.)
- [~] Port encrypted secret compatibility. (Rust now performs tenant-authorized repository/instance-allowlist lookup, exact declaration completeness, byte-compatible Erlang external-term AAD and versioned AES-256-GCM encryption/decryption, write-only CSRF-protected maintainer administration, atomic audit, zeroizing runtime values, value-free Docker arguments, job/service environment injection, and independent boundary-safe stdout/stderr redaction. Key rotation and telemetry parity remain.)
- [~] Port source-control providers, runner protocol, execution adapters, secrets, and storage. (Exact-SHA provider checkout, local Docker execution, service containers, encrypted runtime secrets, local/S3 cache and artifact built-ins, validated backend selection and migration acknowledgement, reference-safe backend-neutral retention/reconciliation, and authenticated attempt-scoped remote source, secret, log, cache, and artifact transfers are implemented in Rust. The Actix WebSocket adapter authenticates upgrade headers, bounds frames, negotiates Phoenix-compatible protocol-v1 hello/welcome sessions, persists bounded software/capability metadata, reconciles active attempts, delivers heartbeat acknowledgements, pending offers, and cancellations, and durably processes job accept/reject, ordered attempt events, and log events with protocol acknowledgements and conflict diagnostics. Administrator-only enrollment-token creation, atomic one-time enrollment, digest-only credentials, five-minute-overlap rotation, immediate revocation, audit rows, no-store Actix secret delivery, tenant-scoped fleet connectivity/capacity/configuration APIs, bounded per-source enrollment throttling, bounded per-source/runner WebSocket authentication throttling, secret-free authentication anomaly audits, and a responsive CSRF-protected Actix fleet browser are implemented. GitHub, GitLab, and Forgejo webhook routes bound raw bodies and headers, authenticate secrets in constant time before JSON decoding, and atomically persist deduplicated provider delivery identities with durable processing jobs. Pure provider-neutral normalization covers exact-SHA pushes, tags, and same-repository non-draft change requests while explicitly ignoring irrelevant actions. The runtime claims those jobs, resolves trusted repositories by provider instance and identity, fetches the exact commit, resolves canonical workflows, matches branch/tag triggers, creates tenant-scoped idempotent pipelines, retries transient failures, and finalizes delivery state. Provider discovery parity, real reverse-proxy WebSocket acceptance, and remaining delivery surfaces remain.)
- [~] Replace LiveView with an accessible real-time Actix-delivered browser experience. (Actix now owns the public home, sign-in/setup/sign-out, pipeline list/detail with jobs, immutable workflow source, job attempts/artifacts, bounded structured log navigation, complete log and artifact downloads, repository history, exact-SHA manual workflow discovery/launch, scheduled-workflow discovery, encrypted-secret administration, build-information, administrator, dynamic Prometheus metrics, database-backed build/coverage badges, and cookie-authenticated SSE refresh through a local JavaScript bundle. Fine-grained in-place DOM updates and remaining operational administration views remain.)
- [~] Port CLI, release packaging, configuration, telemetry, and operational tooling. (A native `robine` Rust binary owns version reporting, safe preview/write initialization, shared-parser validation, stable JSON diagnostics, local Docker job/dependency execution, bounded repository snapshots, live output, and documented exit classes. The standalone `robine-runner` Rust binary performs secure enrollment, private configuration, authenticated protocol-v1 session registration, heartbeat/offer polling, source and secret acquisition, Docker execution, log streaming, and terminal event reporting. The Rust packager builds versioned optimized CLI/server/runner binaries, sets executable modes, and writes verified SHA-256 checksums. Remaining installation and operational tooling remain.)
- [ ] Pass specification parity, upgrade, rollback, and release acceptance suites.
- [x] Remove the Elixir/Phoenix implementation and build dependencies after running the final 396-test compatibility witness and the Rust unit, integration, PostgreSQL, and native-package suites.
