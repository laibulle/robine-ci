...
- [x] Add CI gates for Rust formatting, strict Clippy, workspace and PostgreSQL compatibility tests, coverage, RustSec advisories, SPDX license policy, source provenance, and dependency-policy checks through `cargo deny`.
...
- [x] Port CLI, release packaging, configuration, telemetry, and operational tooling. (Native CLI/server/runner binaries, secure runner enrollment/session execution, Rust-owned schema bootstrap, Actix probes/metrics, native overlays, platform-specific packaging, executable modes, generated locked dependency notices, and verified SHA-256 manifests are implemented and smoke-tested.)
