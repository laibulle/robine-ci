...
- [~] Port source-control providers, runner protocol, execution adapters, secrets, and storage. (Exact-SHA provider checkout, local Docker execution, service containers, encrypted runtime secrets, local/S3 cache and artifact built-ins, validated backend selection and migration acknowledgement, reference-safe backend-neutral retention/reconciliation, and authenticated attempt-scoped remote source, secret, log, cache, and artifact transfers are implemented in Rust. Provider webhook/discovery parity, WebSocket session negotiation, runner enrollment/credential administration and rotation, and remaining delivery surfaces remain.)
