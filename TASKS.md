...
- [~] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, and graph reconciliation are implemented. The native minute worker now evaluates pure UTC cron expressions, fetches every trusted repository once per scan, creates exact-SHA `system:scheduler` pipelines with stable occurrence identities, retains a compare-and-set PostgreSQL cursor on failure, catches up at most 1,440 minutes, exposes truncation in its result, and runs independently per tenant. Rust runtime workers claim SQL outbox, durable dispatch, local execution, and GitHub projection work with capped retry/dead-letter and reconciliation semantics. A framework-independent Rust execution crate owns the hardened Docker lifecycle, exact-SHA source, services, secrets, cache, artifacts, cancellation, logs, and cleanup. The pure Rust workflow crate owns validated schema-v1 composition, matrices, inputs, schedules, and canonical durable graphs. Authenticated HTTP/WebSocket remote delivery is durable and attempt-scoped, and a real TCP reverse-proxy test proves the compatible upgrade and hello/welcome exchange. Dependency reruns remain.)
