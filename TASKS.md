...
- [~] Port source-control providers, runner protocol, execution adapters, secrets, and storage. (Exact-SHA provider checkout, local Docker execution, service containers, encrypted runtime secrets, local cache/artifact built-ins, and reference-safe local retention/reconciliation are implemented in Rust. Provider webhook/discovery parity, remote transfers, S3, administration, and remaining delivery surfaces remain.)
