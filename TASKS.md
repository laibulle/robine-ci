...
- [~] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, and graph reconciliation are implemented. A Rust runtime worker now claims SQL outbox events concurrently, applies idempotent internal projections, atomically enqueues unique durable dispatch jobs, retries with capped stable jitter, and dead-letters after ten failures. Workflow parsing, durable dispatch-job execution/recovery, offer push transport and attempt transfers, dependency reruns, and external projection delivery remain.)
