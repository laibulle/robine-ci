...
- [~] Port source-control providers, runner protocol, execution adapters, secrets, and storage. (Exact-SHA provider checkout, local Docker execution, service containers, encrypted runtime secrets, and local cache/artifact built-ins are implemented in Rust. Provider webhook/discovery parity, remote transfers, S3, retention/GC workers, administration, and remaining delivery surfaces remain.)
