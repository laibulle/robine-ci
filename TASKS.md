...
- [~] Replace LiveView with an accessible real-time Actix-delivered browser experience. (Actix now owns the public home, sign-in/setup/sign-out, pipeline list/detail with jobs, immutable workflow source, job attempts/artifacts, complete log and artifact downloads, repository history, encrypted-secret administration, build-information, administrator, metrics, and badge route surface with cookie authentication and responsive server-rendered HTML. Manual/scheduled repository workflow controls, richer bounded log navigation, and browser real-time updates remain.)
