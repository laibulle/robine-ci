...
- [x] Inventory HTTP, database, background-work, browser, CLI, and runner compatibility surfaces. (The final Phoenix router witness is mapped to Actix in `docs/operations/rust-compatibility-inventory.md`; historical/fresh PostgreSQL, worker, native runner, CLI, package, browser, probe, badge, metrics, webhook, and transfer evidence is linked to executable suites.)
...
- **Spec:** [PLAT-004](docs/specs/platform/plat-004-rust-actix-migration.md)
- **Depends on:** RUST-001
- [x] Port database access, transactions, tenant isolation, identity, and sessions. (SQLx tenant transactions, first-admin bootstrap, compatible local sign-in, cookie and bearer session lifecycle, OIDC discovery/PKCE/callback validation with collision-safe provisioning and one-shot JWKS rotation recovery, administrator-only role administration with last-admin protection, and authenticated projections are implemented and tested.)
- [x] Port workflows, pipeline lifecycle, scheduling, durable jobs, and outbox delivery. (Tenant-scoped lifecycle, local/remote scheduling, runner event/reconnect/offer flows, graph reconciliation, the bounded durable UTC scheduler, SQL outbox/dispatch/projection workers, hardened Docker execution, exact-SHA source, services, secrets, cache, artifacts, cancellation, logs, cleanup, workflow composition, matrices, inputs, schedules, remote delivery, reconnect recovery, and dependency reruns are implemented and covered by pure, PostgreSQL, HTTP, and reverse-proxy tests.)
- [x] Port encrypted secret compatibility. (Rust performs tenant-authorized repository/instance-allowlist lookup, exact declaration completeness, byte-compatible Erlang external-term AAD and versioned AES-256-GCM encryption/decryption, write-only CSRF-protected administration, atomic audit, zeroizing runtime values, job/service injection, and boundary-safe redaction. Administrator-only bounded cursor rotation preserves mixed-version reads, conditionally re-encrypts and audits each record atomically, resumes after interruption, returns no secret material, and exposes target-version/pending metrics.)
- [~] Port source-control providers, runner protocol, execution adapters, secrets, and storage. (Exact-SHA provider checkout, local Docker execution, service containers, encrypted runtime secrets, local/S3 cache and artifact built-ins, validated backend selection and migration acknowledgement, reference-safe backend-neutral retention/reconciliation, and authenticated attempt-scoped remote source, secret, log, cache, and artifact transfers are implemented in Rust. The Actix WebSocket adapter authenticates upgrade headers, bounds frames, negotiates Phoenix-compatible protocol-v1 hello/welcome sessions, persists bounded software/capability metadata, reconciles active attempts, delivers heartbeat acknowledgements, pending offers, and cancellations, and durably processes job accept/reject, ordered attempt events, and log events with protocol acknowledgements and conflict diagnostics. Administrator-only enrollment-token creation, atomic one-time enrollment, digest-only credentials, five-minute-overlap rotation, immediate revocation, audit rows, no-store Actix secret delivery, tenant-scoped fleet connectivity/capacity/configuration APIs, bounded authentication throttling, and a responsive CSRF-protected Actix fleet browser are implemented. GitHub Checks are projected from durable outbox jobs with stable pipeline/job keys, serialized delivery, installation-token caching, exact-SHA create/update calls, bounded paginated crash recovery, persisted provider IDs, capped retries, hourly repair, and bounded queue metrics. The native GitHub App assistant stores encrypted write-only credentials, discovers every repository in active installations, excludes suspended installations, revalidates the complete candidate tuple before transactional trust/audit, reports effective least-privilege permissions and corrective action, displays the last authenticated repository webhook, and exposes sanitized bounded GitHub API outcome/duration/rate-limit health to administrators and Prometheus. GitHub, GitLab, and Forgejo webhook routes still authenticate and process exact-SHA deliveries, but GitLab/Forgejo outbound status projection is explicitly deferred from the current cutover at the user's request.)
- [~] Replace LiveView with an accessible real-time Actix-delivered browser experience. (Actix now owns the public home, sign-in/setup/sign-out, pipeline list/detail with jobs, immutable workflow source, job attempts/artifacts, bounded structured log navigation, complete log and artifact downloads, repository history, exact-SHA manual workflow discovery/launch, scheduled-workflow discovery, encrypted-secret administration, build-information, five URL-addressable administration areas, dynamic Prometheus metrics, database-backed build/coverage badges, and cookie-authenticated SSE refresh through a local JavaScript bundle. Fine-grained in-place DOM updates remain an enhancement; the independent screen-reader acceptance session remains an external release gate.)
...
- [~] Pass specification parity, upgrade, rollback, and release acceptance suites. (Rust parity, PostgreSQL compatibility, native packaging, checksum, upgrade, and documented rollback gates pass; the two external human/GitHub acceptance records remain.)
...
- [~] Resolve every applicable unchecked acceptance criterion in accepted specs. (All locally executable Rust contract gates pass; only the independent accessibility session and measured external GitHub first-pipeline session remain.)
  - External evidence gate: `robine verify-acceptance --first-pipeline FILE --accessibility FILE --artifact-manifest SHA256SUMS`.
