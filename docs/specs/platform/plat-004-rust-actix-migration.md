...
- [x] Rust owns framework-independent schema-v1 YAML parsing for the shared workflow fixture corpus, returns stable source-located diagnostics, validates canonical workflow paths and selected triggers, expands deterministic matrix jobs and fan-in dependencies, injects declared manual or reusable inputs, and derives the durable pipeline graph from the immutable revision source.
