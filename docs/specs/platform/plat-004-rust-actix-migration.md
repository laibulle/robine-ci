...
- **FR-8:** The current cutover MUST preserve GitHub webhook, exact-SHA source, and Checks behavior. GitLab and Forgejo outbound status projections are deferred and are not a cutover gate for this increment.
...
- [x] Rust accepts authenticated bounded Actix WebSocket upgrades on the compatible Phoenix transport path, negotiates protocol v1 through hello/welcome, persists bounded software and capability metadata, reconciles reported attempts, emits heartbeat acknowledgements, pending offers, and cancellations, and durably processes accept/reject, ordered attempt, and log events with compatible acknowledgements and conflict diagnostics; a real TCP reverse-proxy handshake test proves upgrade-header and frame passthrough.
- [x] Every legacy production browser, authentication, source-control webhook, runner-transfer, health, badge, and metrics route is registered under its original HTTP method and exercised by an Actix route-manifest test; native-only workflow, SSE, administration, and runner protocol routes remain additive.
- [x] GitHub App discovery uses App and installation authentication, excludes suspended installations, traverses bounded pagination, revalidates repository ID, installation ID, and full name before trust, atomically audits trust, stores private-key and webhook credentials as encrypted write-only instance secrets, and displays live least-privilege permission diagnosis.
- [x] GitHub API requests retain only sanitized latest operation/outcome/status-class and primary rate-limit state plus bounded operation/outcome request-duration counters; administrator health, repository last-webhook activity, and Prometheus output expose no URLs, repository names, payloads, credentials, or arbitrary labels.
...
- [x] Rust owns bounded UTC schedule reconciliation with conventional day-field semantics, one exact-SHA source fetch per trusted repository, deterministic occurrence pipelines, a tenant-scoped compare-and-set cursor, retained failures, 1,440-minute catch-up/truncation, and a shutdown-aware minute worker.
- [x] Rust projects GitHub pipeline and job checks through serialized durable jobs with stable external keys, exact-SHA creation, persisted provider IDs, bounded paginated crash recovery, capped retry/dead-letter behavior, hourly repair, and queue/scheduler metrics; no provider call runs inside a database transaction.
...
- [x] Every accepted domain contract has Rust unit and integration coverage, enforced by the executable coverage index in `docs/operations/rust-contract-coverage.md`.
- [x] Existing PostgreSQL records and the final historical schema pass Rust compatibility tests; fresh databases bootstrap from the Rust-owned baseline and older incomplete schemas fail closed.
- [x] Every HTTP and runner route passes request/response parity tests.
- [x] Durable jobs, outbox delivery, source-control ingestion, local execution handoff, remote receipts, status projection, schedule cursors, leases, and reconciliation pass stale-claim, crash-window, concurrent-claim, exact-replay, conflict, retry, dead-letter, and duplicate tests.
- [x] Browser handler tests and native CLI, server, runner package/checksum/version smokes pass against Rust artifacts.
- [x] Production cutover and rollback are documented; an isolated native smoke rehearses empty-schema bootstrap, both HTTP probes, clean shutdown, and restart on the resulting schema. A restored production-like backup rehearsal remains an operator release gate.
- [x] Elixir/Phoenix sources and build dependencies are removed.

## Open questions

- None blocking. Actix renders semantic HTML and a local JavaScript bundle consumes authenticated SSE notifications, then reloads durable projections.

## Out of scope / future work

- Behavioral changes unrelated to the runtime migration.
- GitLab and Forgejo outbound status projection parity, explicitly deferred for this cutover increment.
