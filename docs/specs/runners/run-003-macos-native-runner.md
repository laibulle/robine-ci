...
- **FR-14:** Every Robine instance MUST expose the cross-platform installer defined by RUN-005. On macOS it MUST select the released Darwin binary and delegate config-bound LaunchAgent reconciliation to `rbe install` without privilege escalation.
