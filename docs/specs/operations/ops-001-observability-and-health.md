...
The application boundary exposes administrator-authorized operational queries backed by the PostgreSQL and provider adapters. Public Actix handlers project only overall readiness; focused administrator pages render complete secret-free results. Liveness remains a delivery-only process probe because it intentionally performs no application dependency work.
